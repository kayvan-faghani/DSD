...
module cordic(
    ...
    output  [`WORDLENGTH-1:0]   cos_out,
	 output  [`CORDIC_STAGES:0] [`WORDLENGTH-1:0] x_array_out,
	 output  [`CORDIC_STAGES:0] [`WORDLENGTH-1:0] y_array_out,
	 output  [`CORDIC_STAGES:0] [`WORDLENGTH-1:0] z_array_out
);
    localparam real SCALE = 2.0**20;

//    function automatic logic [`CORDIC_STAGES-1:0] [`WORDLENGTH-1:0] gen_atans();
...
//    localparam logic [`WORDLENGTH-1:0] k_inv = gen_inv_k();
    ...
    generate
        for (i=0; i < `CORDIC_STAGES; i++) begin : cordic_step
				...
				assign x_array[i+1] = z_sign 							?	 
                                    x_array[i] + shift_y		: 
                                    x_array[i] - shift_y 	;
            
            assign y_array[i+1] = z_sign 							?
                                    y_array[i] - shift_x 	:
                                    y_array[i] + shift_x		;
            
            assign z_array[i+1] = z_sign							?
                                    z_array[i] + atans[i] 	:
                                    z_array[i] - atans[i]	;
        end
    endgenerate
    ...
    assign cos_out = x_array[`CORDIC_STAGES];
	 assign x_array_out = x_array;
	 assign y_array_out = y_array;
	 assign z_array_out = z_array;
endmodule